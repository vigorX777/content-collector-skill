...
### 模式2：自动检测链接
当用户发送链接时，即使没有说"收藏"等关键词，也会自动检测并执行收录：

**支持的链接类型：**
- X/Twitter: `x.com/i/status/*`, `twitter.com/*/status/*`
- 微信公众号: `mp.weixin.qq.com/s/*`
- 飞书: `*.feishu.cn/docx/*`, `*.feishu.cn/wiki/*`, `*.feishu.cn/base/*`
- 通用网页: 其他 URL
...
| 微信公众号 | mp.weixin.qq.com | web_fetch |
| 飞书 | feishu.cn | feishu_fetch_doc |
| 小红书 | xiaohongshu.com | web_fetch |
| 抖音/视频号 | douyin.com | web_fetch |
| 知乎 | zhihu.com | web_fetch |
| 掘金 | juejin.cn | web_fetch |
| 通用网页 | 其他 | web_fetch |
...
| 关键词 | 分类 |
|--------|------|
| prompt、agent、model、LLM、GPT | 📖 技术教程 |
| 产品、功能、需求、用户体验 | 💡 产品想法 |
| 工具、软件、app、插件 | 🔧 工具推荐 |
| 热点、新闻、事件 | 🔥 热点资讯 |
| 案例、实战、项目 | 🛠️ 实战案例 |
...
```json
{
  "filter": {
    "conjunction": "or",
    "conditions": [
      {"field_name": "原文链接", "operator": "contains", "value": ["当前URL"]}
    ]
  }
}
```
...
在正式使用本技能前，需要完成飞书用户授权，以确保创建的内容归用户所有。
...
#### 步骤 4：开始使用
授权成功后，后续收藏操作将自动以用户身份执行。

**注意事项：**
- 授权只需进行一次，有效期约 30 天
- 授权后创建的多维表格/文档归用户所有
- 如遇到权限问题，重新执行步骤 2-3 即可

---

### 预检流程 (Pre-flight Check)
每次执行收藏前，系统会自动检查：
1. **UAT 有效性**：检查用户授权是否过期
2. **Table 可访问性**：验证多维表格是否可写
3. **自动引导**：如需授权，自动发送授权卡片
